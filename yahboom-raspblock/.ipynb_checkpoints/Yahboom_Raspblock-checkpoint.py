...
#     matDst = np.float32([[100,240], [200,240], [200,0], [100,0]])
    ...
    while 1:
        ...
        if g_mode == 'target_track':
            ...
            if g_tag_identify_switch == 'open':
                if g_tag_select == 'qrcode':
                    ...
                    try:
                        ...
                        #Logger('error.log', level='error').logger.error('未识别到')
                        #print('识别结果：什么也没识别到哦~' )
                        ...
                        #cv2.putText(frame, 'unidentification', (10,80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,200), 2, cv2.LINE_AA) #只能显示英文
                        ...
                        #cv2.putText(frame, res, (10,80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,200), 2, cv2.LINE_AA) #只能显示英文   
            ...
            dst_retval, dst_binaryzation = cv2.threshold(dst_gray, 128, 255, cv2.THRESH_BINARY) # 100->120  阈值设置
...
#             rightx_base_value = np.max(histogram[midpoint:],axis = 0)
            leftx_base = np.argmin(histogram[:midpoint],axis = 0) + 5              #加上黑线的宽度
            rightx_base = np.argmin(histogram[midpoint:],axis = 0) + midpoint + 5  #加上黑线宽度
