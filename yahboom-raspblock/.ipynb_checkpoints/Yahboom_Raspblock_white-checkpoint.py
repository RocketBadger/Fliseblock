...
@LastEditors  : Liusen
@LastEditTime : 2020-03-23 16:15:03  巡白线驾驶
'''
...
#                     robot.Servo_control(target_valuex,target_valuey)
                    ...
                    if updownpulse != 1500:
                        ...
                        target_valuey = updownpulse  #刘森调试  2000  2100 
                    else:
                        target_valuex = 1550
                        target_valuey = 2050
        ...
        if g_mode == 'remote_control' or g_mode == 'mecanum_control':
            ...
            if g_presentation_mode == 'around':
                ...
                for i in range(1, 2000):
                    robot.Speed_Wheel_control(-5, 20, -20, 5)    #右平移  
                
                robot.Speed_Wheel_control(0, 0, 0, 0)    #停止
                time.sleep(2)
                
            elif g_presentation_mode == 'translation':
                print("translation")
                for i in range(1, 1000):
                    robot.Speed_Wheel_control(-8, 8, -8, 8)    #右平移
                robot.Speed_Wheel_control(0, 0, 0, 0)      #停止
                time.sleep(2)
                for i in range(1, 1000):
                    robot.Speed_Wheel_control(-8, -8, -8, -8)  #所有轮子向后2的速度
                robot.Speed_Wheel_control(0, 0, 0, 0)      #停止
                time.sleep(2)
                for i in range(1, 1000):
                    robot.Speed_Wheel_control(8, -8, 8, -8)    #左平移
                robot.Speed_Wheel_control(0, 0, 0, 0)      #停止
                time.sleep(2)
                for i in range(1, 1000):
                    robot.Speed_Wheel_control(8, 8, 8, 8)      #所有轮子向前2的速度
                robot.Speed_Wheel_control(0, 0, 0, 0)      #停止
                time.sleep(2)
...
#     matDst = np.float32([[100,240], [200,240], [200,0], [100,0]])
    ...
    matSrc = np.float32([[0, 149],  [320, 149], [281, 72], [43, 72]]) #310-320
    matDst = np.float32([[0,240], [320,240], [320,0], [0,0]])          #310-320
        ...
        if g_mode == 'target_track':
            ...
            if g_tag_identify_switch == 'open':
                ...
                #       cv2.putText(frame, '未识别', (250,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,200), 2, cv2.LINE_AA) #只能显示英文
                        frame = cv2ImgAddText(frame, "未识别到", 10, 30, (0, 0 , 255), 30)
                        info = "未识别到"
                    else:
                        print('识别结果：' + hand[res])
                        info = hand[res]
            ...
            dst_retval, dst_binaryzation = cv2.threshold(dst_gray, 120, 255, cv2.THRESH_BINARY) # 100->120
...
#                 rightx_base = 315
            ...
            if rightx_base > 310:
                rightx_base = 310
            if leftx_base > 150 or rightx_base - leftx_base < 50:
                leftx_base = 10
            if rightx_base < 169 or rightx_base - leftx_base < 50:
                rightx_base = 310

            histogram = np.sum(dst_binaryzation[dst_binaryzation.shape[0]//2:,:], axis=0)
            dst_binaryzation = cv2.cvtColor(dst_binaryzation,cv2.COLOR_GRAY2RGB)
            cv2.line(dst_binaryzation,(159,0),(159,240),(255,0,255),2) # 149-159 fix liusen 200323
            ...
            Bias = 159 - lane_center  # 149-159 fix liusen 200323
...
#             if g_presentation_mode == 'around':
#                 print("around")
#                 robot.Speed_Wheel_control(-8, -15, 15, 8)    #右环绕
#                 time.sleep(5)
#                 robot.Speed_Wheel_control(0, 0, 0, 0)    #停止
#                 time.sleep(5)
#                 robot.Speed_Wheel_control(8, 15, -15, -8)    #左环绕
#                 time.sleep(5)
#                 robot.Speed_Wheel_control(0, 0, 0, 0)    #停止
#                 time.sleep(5)
#             elif g_presentation_mode == 'translation':
#                 print("translation")
#                 robot.Speed_Wheel_control(-8, -8, 8, 8)    #右旋
#                 time.sleep(2)
#                 robot.Speed_Wheel_control(-8, -8, -8, -8)  #所有轮子向后2的速度
#                 time.sleep(2)
#                 robot.Speed_Wheel_control(8, 8, -8, -8)    #左旋
#                 time.sleep(2)
#                 robot.Speed_Wheel_control(8, 8, 8, 8)      #所有轮子向前2的速度
#                 time.sleep(2)
#                 robot.Speed_Wheel_control(0, 0, 0, 0)      #停止
